...
export default function AdminLayout({
    ...
    }, [isNotificationsOpen]); // Re-check when opening dropdown
        ...
        <div className="flex h-screen bg-[#0f172a] text-white font-sans overflow-hidden p-0 md:p-4 gap-4 selection:bg-indigo-500 selection:text-white relative">
            ...
            >
                ...
                <nav className="flex-1 px-4 py-6 space-y-3 overflow-y-auto relative z-10 scrollbar-hide">
                    <NavItem href="/admin/dashboard" icon={<LayoutDashboard size={20} />} label="Overview" isOpen={isSidebarOpen || isMobile} isActive={pathname === '/admin/dashboard'} delay={0.1} />
                    <NavItem href="/admin/collections" icon={<Wallet size={20} />} label="Collections" isOpen={isSidebarOpen || isMobile} isActive={pathname === '/admin/collections'} delay={0.2} />
                    <NavItem href="/admin/staff" icon={<Users size={20} />} label="Field Staff" isOpen={isSidebarOpen || isMobile} isActive={pathname === '/admin/staff'} delay={0.3} />
                    <NavItem href="/admin/customers" icon={<User size={20} />} label="Customers" isOpen={isSidebarOpen || isMobile} isActive={pathname === '/admin/customers'} delay={0.35} />
                    <NavItem href="/admin/expenses" icon={<TrendingDown size={20} />} label="Expenses" isOpen={isSidebarOpen || isMobile} isActive={pathname === '/admin/expenses'} delay={0.38} />
                    <div className="my-4 h-px bg-white/5 mx-4"></div>
                    <NavItem href="/admin/reports" icon={<FileText size={20} />} label="Reports" isOpen={isSidebarOpen || isMobile} isActive={pathname === '/admin/reports'} delay={0.4} />
                    <NavItem href="/admin/settings" icon={<Settings size={20} />} label="Settings" isOpen={isSidebarOpen || isMobile} isActive={pathname === '/admin/settings'} delay={0.5} />
                </nav>
                    ...
                    {(isSidebarOpen || isMobile) ? (
                        <>
                            ...
                            <div className="text-center opacity-40 hover:opacity-80 transition-opacity">
                                ...
                                <p className="text-xs font-extrabold text-indigo-400 tracking-wide mt-0.5">AARYA TECHNOLOGIES</p>
                            </div>
                        </>
                    ) : (
            ...
            <main className="flex-1 bg-[#0f172a] md:rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col relative border border-white/5 z-0">
                                ...
                                {isNotificationsOpen && (
                                    ...
                                    >
                                        ...
                                        <div className="max-h-[300px] overflow-y-auto">
                                            {[
                                                { id: 1, type: 'newPayment', title: "Payment Received", desc: "Received ₹5,000 from Shiv Shakti Traders", time: "2 min ago", icon: <Wallet size={14} className="text-emerald-400" />, bg: "bg-emerald-500/10" },
                                                { id: 2, type: 'dailyReport', title: "Daily Target Achieved", desc: "Rahul Varma crossed daily collection goal", time: "1 hour ago", icon: <TrendingUp size={14} className="text-blue-400" />, bg: "bg-blue-500/10" },
                                                { id: 3, type: 'systemUpdates', title: "System Update", desc: "Maintenance scheduled for tonight", time: "5 hours ago", icon: <Settings size={14} className="text-slate-400" />, bg: "bg-slate-500/10" },
                                            ]
                                                .filter(n => notificationPreferences[n.type as keyof typeof notificationPreferences] !== false)
                                                .map((n) => (
                ...
                {/* Content Scroll Area */}
                <div className="flex-1 overflow-y-auto p-4 md:p-8 pt-0 scrollbar-thin scrollbar-thumb-indigo-900/50 hover:scrollbar-thumb-indigo-700/50">
                    {children}
                </div>
...
function NavItem({ icon, label, isOpen, href, isActive, delay }: any) {
    return (
