...
import { Save, User, Bell, Shield, Database, Layout, Building2, Smartphone, Lock, History, Upload, Mail, Globe, CreditCard, MapPin, Plus, Trash2 } from "lucide-react";
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <InputGroup label="Application Name" defaultValue="Payment Soft Admin" placeholder="Enter app name" />
                <SelectGroup label="Currency" options={['INR (₹)', 'USD ($)', 'EUR (€)']} defaultValue="INR (₹)" />
                <SelectGroup label="Language" options={['English', 'Gujarati', 'Hindi']} defaultValue="English" />
                <SelectGroup label="Timezone" options={['(GMT+05:30) India Standard Time', '(GMT+00:00) UTC', '(GMT-05:00) Eastern Time']} defaultValue="(GMT+05:30) India Standard Time" />
                <SelectGroup label="Date Format" options={['DD/MM/YYYY', 'MM/DD/YYYY', 'YYYY-MM-DD']} defaultValue="DD/MM/YYYY" />
                <SelectGroup label="Financial Year Start" options={['April 1st', 'January 1st']} defaultValue="April 1st" />
                <SelectGroup label="Theme Preference" options={['Dark Mode', 'Light Mode', 'System Default']} defaultValue="Dark Mode" />
                <SelectGroup label="Default Rows Per Page" options={['10', '25', '50', '100']} defaultValue="10" />
            </div>
...
function CompanySettings() {
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <InputGroup label="Company Name" defaultValue="Aarya Technologies" />
                <InputGroup label="GST / VAT Number" defaultValue="24ABCDE1234F1Z5" />
                <InputGroup label="Email Address" defaultValue="admin@aaryatech.com" icon={<Mail size={16} />} />
                <InputGroup label="Website" defaultValue="www.aaryatech.com" icon={<Globe size={16} />} />
                <div className="md:col-span-2">
                    <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Business Address</label>
                    <textarea rows={3} className="w-full bg-[#0f172a] border border-white/10 rounded-xl px-5 py-4 text-white outline-none focus:border-indigo-500 transition-all font-medium placeholder:text-slate-600 resize-none" defaultValue="405, Silicon Valley, Near Shivranjani Cross Roads, Satellite, Ahmedabad - 380015"></textarea>
                </div>
            </div>
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="space-y-4">
                <ToggleItem title="Daily Collection Report" description="Receive a daily summary of total collections via SMS/Email." defaultChecked />
                <ToggleItem title="New Payment Alerts" description="Get notified instantly when a staff member adds a new entry." defaultChecked />
                <ToggleItem title="Staff Login Alerts" description="Notify admin when staff members log in to their app." />
                <ToggleItem title="Low Balance Warnings" description="Alert when customer credit limit is exceeded." defaultChecked />
                <ToggleItem title="System Updates" description="Receive notifications about software updates and maintenance." defaultChecked />
            </div>
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="bg-orange-500/10 border border-orange-500/20 p-6 rounded-2xl md:w-2/3">
                ...
                <div className="space-y-4">
                    <input type="password" placeholder="Current Password" className="w-full bg-[#0f172a] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-orange-500/50 transition-all" />
                    <input type="password" placeholder="New Password" className="w-full bg-[#0f172a] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-orange-500/50 transition-all" />
                    <input type="password" placeholder="Confirm New Password" className="w-full bg-[#0f172a] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-orange-500/50 transition-all" />
                    <button className="bg-orange-600 hover:bg-orange-500 text-white px-6 py-2 rounded-xl font-bold text-sm shadow-lg shadow-orange-600/20 transition-all">Update Password</button>
                </div>
            </div>

            <div className="space-y-4">
                <ToggleItem title="Two-Factor Authentication (2FA)" description="Require an OTP code in addition to password for login." />
                <ToggleItem title="Force Logout Inactive Staff" description="Automatically logout staff apps after 1 hour of inactivity." defaultChecked />
            </div>
        </div>
    )
}
...
function BackupSettings() {
    return (
        <div className="space-y-8">
            ...
            <div className="pt-6">
                <button className="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-4 rounded-2xl font-bold text-lg shadow-xl shadow-indigo-500/20 transition-transform active:scale-95 flex items-center gap-3">
                    <Upload size={20} />
                    Backup Now
                </button>
...
/* Helper Components */
function InputGroup({ label, defaultValue, placeholder, icon }: any) {
    return (
    ...
    return (
        <div className="flex items-center justify-between p-4 bg-[#0f172a] rounded-xl border border-white/5">
            ...
            <button
                ...
                className={`w-14 h-8 rounded-full transition-colors relative ${enabled ? 'bg-emerald-500' : 'bg-slate-700'}`}
            >
                <div className={`w-6 h-6 bg-white rounded-full absolute top-1 transition-transform ${enabled ? 'left-7' : 'left-1'}`}></div>
            </button>
        </div>
    )
}
...
function SaveChangesButton() {
    return (
        <button className="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-4 rounded-2xl font-bold flex items-center gap-3 shadow-xl shadow-indigo-500/20 transition-transform active:scale-95 group">
