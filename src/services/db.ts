    ...
    const mergeAndSave = (key: string, remoteData: any[], eventName: string) => {
        try {
            ...
            try {
                ...
                console.warn(`Error parsing local ${key} during sync, resetting to empty`, e);
                local = [];
            }
            ...
            // Remote data (latest from server) should overwrite local base data, 
            // BUT careful with detailed merges. Here we trust server as source of truth for conflict 
            // if we assume server has latest. 
            // However, for offline support, we just ensuring we have ALL items.
            ...
            if (eventName === 'transaction-updated') {
                console.error("CRITICAL: Collections sync failed. Check Firestore Rules/Auth.");
            }
        }
    };

    // DEBUG: Alert on Connection State
    /*
    const connectedRef = doc(firestore, ".info/connected");
    onSnapshot(connectedRef, (snap) => {
        if (snap.data()?.connected === false) {
             console.warn("⚠️ Firestore Offline");
        } else {
...
onSnapshot(collection(firestore, "dealers"), (snapshot) => {
    ...
    if (data.length > 0) {
        mergeAndSave(STORAGE_KEYS.DEALERS, data, 'expense-updated');
    }
...
// 10. Cross-Tab Sync (LocalStorage Events)
window.addEventListener('storage', (e) => {
    if (e.key === STORAGE_KEYS.CUSTOMERS) window.dispatchEvent(new Event('customer-updated'));
    if (e.key === STORAGE_KEYS.EXPENSES) window.dispatchEvent(new Event('expense-updated'));
    if (e.key === STORAGE_KEYS.COLLECTIONS) window.dispatchEvent(new Event('transaction-updated'));
    if (e.key === STORAGE_KEYS.STAFF) window.dispatchEvent(new Event('staff-updated'));
    if (e.key === STORAGE_KEYS.DEALERS) window.dispatchEvent(new Event('expense-updated')); // Dealers share expense/party event? Or add dealer-updated. using expense for now as likely shared context.
    if (e.key === STORAGE_KEYS.ZONES) window.dispatchEvent(new Event('zone-updated'));
    ...
    saveCustomer: (customer: Customer) => {
        ...
        try {
            try {
                ...
                console.error("❌ Firebase Sync Error - Immediate:", e);
            }
