...
import { Filter, Download, Search, Check, ChevronLeft, ChevronRight, MoreHorizontal, ArrowUpRight, TrendingUp, Calendar, CreditCard, DollarSign, X, Clock, Phone, FileText, Edit, Trash2, Eye, Plus, Wallet, ArrowRight } from "lucide-react";
...
export default function CollectionsPage() {
    ...
    const itemsLength = filteredTransactions.length;
    const totalPages = itemsLength > 0 ? Math.ceil(itemsLength / itemsPerPage) : 1;
        ...
        <div className="w-full space-y-8 relative">
            ...
            <div className="flex flex-col gap-6">
                <div className="flex justify-between items-end">
                    ...
                    <div className="bg-[#0f172a] p-1.5 rounded-xl border border-white/10 flex items-center shadow-2xl shadow-indigo-500/10">
                        <button
                            onClick={() => setViewMode('Collections')}
                            className={`px-6 py-3 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${viewMode === 'Collections' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                        >
                            <Wallet size={16} />
                            Collections
                        </button>
                        <button
                            ...
                            className={`px-6 py-3 rounded-lg text-sm font-bold transition-all flex items-center gap-2 relative ${viewMode === 'Handovers' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                        >
                            <ArrowRight size={16} />
                            Handovers
                            {/* Notification Dot */}
                    ...
                    <button
                        ...
                        className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-indigo-500/20 active:scale-95 transition-all"
                    >
