                ...
                {activeTab === 'transactions' ? (
                    ...
                    >
                        ...
                        <div className="md:hidden flex flex-col p-4 space-y-4">
                            {filteredExpenses.length > 0 ? (
                                filteredExpenses.map((expense) => (
                                    <div key={expense.id} className="bg-white/5 border border-white/5 rounded-2xl p-4 active:scale-98 transition-transform">
                                        <div className="flex justify-between items-start mb-3">
                                            <div>
                                                <p className="text-white font-bold text-lg leading-tight">{expense.party}</p>
                                                <p className="text-slate-400 text-xs font-medium mt-0.5">{expense.title}</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-white font-extrabold text-xl tracking-tight">₹ {expense.amount.toLocaleString('en-IN')}</p>
                                                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">{expense.method}</p>
                                            </div>
                                        </div>

                                        <div className="flex justify-between items-center pt-3 border-t border-white/5">
                                            <div className="flex items-center gap-2">
                                                <span className={`w-2 h-2 rounded-full ${expense.status === 'Paid' ? 'bg-emerald-500' : 'bg-amber-500'}`}></span>
                                                <span className={`text-xs font-bold ${expense.status === 'Paid' ? 'text-emerald-400' : 'text-amber-400'}`}>
                                                    {expense.status}
                                                </span>
                                            </div>
                                            <p className="text-slate-500 text-xs font-bold">{new Date(expense.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</p>
                                        </div>

                                        {/* Mobile Actions */}
                                        <div className="flex justify-end gap-3 mt-3 pt-2 border-t border-white/5 border-dashed">
                                            <button
                                                onClick={() => handleEditExpense(expense)}
                                                className="text-indigo-400 text-xs font-bold flex items-center gap-1 px-2 py-1"
                                            >
                                                <Edit size={14} /> Edit
                                            </button>
                                            <button
                                                onClick={() => handleDelete(expense.id)}
                                                className="text-rose-400 text-xs font-bold flex items-center gap-1 px-2 py-1"
                                            >
