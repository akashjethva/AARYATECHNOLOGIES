...
import { Wallet, Users, ArrowUpRight, Plus, Activity, CreditCard, MoreHorizontal, TrendingUp, Calendar, Filter, X, Check, ChevronDown, Download, AlertCircle, Search, Edit2, Zap, CheckCircle } from "lucide-react";
...
export default function AdminDashboard() {
    ...
    // Staff Live Status State
    const [staffStatus, setStaffStatus] = useState<{ name: string, status: 'Online' | 'Offline', lastUpdated: string } | null>(null);

    // Poll for Staff Status
    useEffect(() => {
        const checkStatus = () => {
            const saved = localStorage.getItem('payment_app_staff_status');
            if (saved) {
                try {
                    setStaffStatus(JSON.parse(saved));
                } catch (e) { }
            }
        };
    ...
    const activeStaffCount = staffStatus?.status === 'Online' ? 1 : 0;
    const totalStaff = 3; // Hardcoded counts
    ...
    const handleApproveHandover = (staffName: string) => {
        showToast(`Approved handover for ${staffName}. Cash balance cleared!`, 'success');
        // In a real app, this would involve a state update or API call.
    };

    const handleSaveTransaction = () => {
        if (!formData.customer || !formData.amount) {
            showToast("Please fill in Customer Name and Amount", 'error');
            return;
        }
    ...
    useEffect(() => {
        ...
        // Load shared transactions from Staff App / Admin Entries
        const savedTransactions = localStorage.getItem('payment_app_transactions');
        if (savedTransactions) {
            try {
                const parsed = JSON.parse(savedTransactions);
                // Merge with initial data (avoid duplicates based on ID if needed, but for now simple merge)
                // We keep initialMockData but put new ones on top
                setTransactions(prev => {
                    // Filter out any that might already be in prev (if using ID)
                    const existingIds = new Set(prev.map(t => t.id));
                    const uniqueNew = parsed.filter((t: any) => !existingIds.has(t.id));
        ...
        <div className="max-w-[1600px] mx-auto space-y-8 relative">
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                ...
                <Card
                    ...
                    onClick={() => setIsHandoverModalOpen(true)}
                />
                <Card title="Active Staff" value={`${activeStaffCount} / ${totalStaff}`} icon={<Activity />} trend={activeStaffCount > 0 ? "Online" : "All Offline"} trendUp={activeStaffCount > 0} color={activeStaffCount > 0 ? "bg-emerald-600" : "bg-slate-600"} href="/admin/staff" />
                <Card title="Month Revenue" value="₹ 8.5L" icon={<Wallet />} trend="+12%" trendUp={true} color="bg-purple-600" href="/admin/reports" />
            </div>

            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">

                {/* Main Feed */}
                <div className="xl:col-span-2 bg-[#1e293b]/40 backdrop-blur-xl rounded-3xl md:rounded-[2.5rem] p-4 md:p-8 border border-white/5 shadow-2xl overflow-hidden flex flex-col h-[600px]">
                    ...
                    <div className="bg-[#0f172a] rounded-[2.5rem] p-8 text-white relative overflow-hidden border border-white/10 shadow-2xl">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="space-y-5">
                                <LeaderboardItem rank="1" name="Rahul Varma" amount="₹ 15k" isOnline={staffStatus?.name === 'Rahul Varma' && staffStatus?.status === 'Online'} />
            ...
            <AnimatePresence>
                {toast.show && (
                    <motion.div
                        ...
                        className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[200] flex items-center gap-3 px-6 py-4 rounded-2xl bg-[#1e293b]/90 backdrop-blur-xl border border-white/10 shadow-2xl min-w-[300px]"
                    >
                        {toast.type === 'success' ? (
                ...
                {isHandoverModalOpen && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        >
                            ...
                            <div className="p-6 space-y-4">
                                <HandoverItem name="Rahul Varma" amount="₹ 5,000" time="Since 2 days" onApprove={() => handleApproveHandover("Rahul Varma")} />
                                <HandoverItem name="Amit Kumar" amount="₹ 4,500" time="Since Yesterday" onApprove={() => handleApproveHandover("Amit Kumar")} />
                                <HandoverItem name="Vikram Singh" amount="₹ 3,000" time="Today" onApprove={() => handleApproveHandover("Vikram Singh")} />
...
function TransactionItem({ time, customer, amount, staff, mode, status, date }: Transaction) {
    return (
        <div className="group flex flex-col md:flex-row md:items-center justify-between p-5 rounded-3xl hover:bg-white/5 border border-transparent hover:border-white/5 transition-all cursor-pointer">
            <div className="flex items-center gap-4 w-full md:w-auto">
                <div className="h-12 w-12 rounded-2xl bg-white/5 flex items-center justify-center font-bold text-slate-400 text-lg group-hover:bg-indigo-600 group-hover:text-white group-hover:shadow-lg transition-all duration-300 shrink-0">
                    {customer[0]}
                </div>
                <div className="flex-1 md:flex-none min-w-0">
                    <h4 className="font-bold text-white group-hover:text-indigo-300 transition-colors truncate">{customer}</h4>
                    <div className="flex flex-wrap items-center gap-2 mt-1">
                        <span className="text-xs font-medium text-slate-500 whitespace-nowrap">{time}</span>
                        {/* Show date if meaningful, otherwise redundant with time */}
                        {date && <span className="text-[10px] text-slate-600 font-bold bg-white/5 px-1 rounded whitespace-nowrap">{date}</span>}
                        <span className="hidden md:inline w-1 h-1 rounded-full bg-slate-600"></span>
                        <span className="text-xs font-bold text-slate-400 whitespace-nowrap">{staff}</span>
            ...
            <div className="w-full md:w-auto flex flex-row md:flex-col justify-between md:justify-end items-center md:items-end mt-4 md:mt-0 pl-[4rem] md:pl-0">
...
function LeaderboardItem({ rank, name, amount, isOnline }: any) {
    return (
        <div className="flex items-center gap-4 p-3 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors cursor-pointer">
            ...
            <div className="flex-1">
                <div className="flex items-center gap-2">
                    ...
                    {isOnline && <span className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_5px_#10b981] animate-pulse"></span>}
                </div>
            </div>
...
function HandoverItem({ name, amount, time, onApprove }: any) {
    return (
        <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5 group hover:bg-white/10 transition-colors">
            ...
            <div className="flex items-center gap-4">
                ...
                <button
                    onClick={onApprove}
                    className="p-2 bg-emerald-500/20 text-emerald-400 rounded-lg hover:bg-emerald-500 hover:text-white transition-all opacity-0 group-hover:opacity-100"
                    title="Approve & Clear Cash"
                >
                    <CheckCircle size={16} />
                </button>
            </div>
